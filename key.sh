...
devicetree_iv=`strings $codename"_"$build_num"_("$device")" | grep keypage-devicetree-iv | cut -d ">" -f 6-6 | cut -d "<" -f 1-1`
devicetree_key=`strings $codename"_"$build_num"_("$device")" | grep keypage-devicetree-key | cut -d ">" -f 5-5 | cut -d "<" -f 1-1`
echo "==========================================="
echo "DEVICETREE IV: "$devicetree_iv
echo "DEVICETREE KEY: "$devicetree_key
